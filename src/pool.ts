...
import { decodeCrocPrice, toDisplayPrice, bigIntToFloat, toDisplayQty, fromDisplayPrice, roundForConcLiq, concDepositSkew, pinTickLower, pinTickUpper, neighborTicks, pinTickOutside, tickToPrice, concBaseSlippagePrice, concQuoteSlippagePrice, GAS_PADDING, AddressZero } from './utils';
...
type BlockTag = number | string | bigint
...
export class CrocPoolView {
    ...
    async spotPrice (block?: BlockTag): Promise<number> {
        let txArgs = block ? { blockTag: toHex(block) } : {}
        let sqrtPrice = (await this.context).query.read.queryPrice
    ...
    async spotTick (block?: BlockTag): Promise<number> {
        let txArgs = block ? { blockTag: toHex(block) } : {}
        return (await this.context).query.read.queryCurveTick
            ([this.baseToken.tokenAddr, this.quoteToken.tokenAddr,
                (await this.context).chain.poolIndex], txArgs)
    }
    ...
    async xykLiquidity (block?: BlockTag): Promise<TokenQty> {
        let txArgs = block ? { blockTag: toHex(block) } : {}
        return (await this.context).query.read.queryLiquidity
            ([this.baseToken.tokenAddr, this.quoteToken.tokenAddr,
                (await this.context).chain.poolIndex], txArgs)
    }
    ...
    async curveState (block?: BlockTag): Promise<{priceRoot_: bigint, ambientSeeds_: bigint, concLiq_: bigint, seedDeflator_: bigint, concGrowth_: bigint}> {
        let txArgs = block ? { blockTag: toHex(block) } : {}
        return (await this.context).query.read.queryCurve
            ([this.baseToken.tokenAddr, this.quoteToken.tokenAddr,
                (await this.context).chain.poolIndex], txArgs)
    }
    ...
    async initPool (initPrice: number): Promise<any> { // TODO: fix any
        ...
        return cntx.dex.write.userCmd([cntx.chain.proxyPaths.cold, calldata], txArgs)
    }
    ...
    async burnAmbientAll (limits: PriceRange, opts?: CrocLpOpts): Promise<any> {
        let [lowerBound, upperBound] = await this.transformLimits(limits)
    ...
    private async sendCmd (calldata: string, txArgs?: { value?: bigint }):
        ...
        Object.assign(txArgs, { gasLimit: gasEst + GAS_PADDING})
        return cntx.dex.write.userCmd([cntx.chain.proxyPaths.liq, calldata], txArgs);
    }

    private async mintAmbient (qty: TokenQty, isQtyBase: boolean,
        limits: PriceRange, opts?: CrocLpOpts): Promise<any> {
    ...
    private async mintRange (qty: TokenQty, isQtyBase: boolean,
        range: TickRange, limits: PriceRange, opts?: CrocLpOpts): Promise<any> {
    ...
    private async ethToAttach (neededQty: TokenQty, opts?: CrocLpOpts): Promise<bigint> {
        if (this.baseToken.tokenAddr !== AddressZero) { return BigInt(0) }
