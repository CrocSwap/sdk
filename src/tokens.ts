...
export class CrocTokenView {
    constructor (context: Promise<CrocContext>, tokenAddr: string) {
        ...
        if (this.isNativeEth) {
            ...
            this.decimals = this.resolve()
                .then(c => c.decimals())
        }
    }

    async approve(): Promise<TransactionResponse | undefined> {
        if (this.isNativeEth) { return undefined }
        const weiQty = BigNumber.from(2).pow(36) // Lots of 0 bytes in calldata to save gas
        return (await this.resolve()).approve((await this.context).dex.address, weiQty)
    }
    ...
    async allowance (address: string): Promise<BigNumber> {
        if (this.isNativeEth) { return MAX_LIQ }
        return (await this.resolve()).allowance(address, (await this.context).dex.address)
    }
